...
export const onFetchTaskById = createAsyncThunk<Task, TaskId>(
  ...
  async (taskId, thunkAPI) => {
    try {
      const res = await todoApi.get(`/tasks/${taskId.id}`)
      return res.data as Task
    } catch (error) {
      return thunkAPI.rejectWithValue(error)
    }
...
export const onFetchTasksByUserId = createAsyncThunk<Task[]>(
  ...
  async (_, thunkAPI) => {
    try {
      const res = await todoApi.get('/tasks/user')
      return res.data as Task[]
    } catch (error) {
      return thunkAPI.rejectWithValue(error)
    }
...
export const onCreateTask = createAsyncThunk<Task, Partial<Task>>(
  ...
  async (newTask, thunkAPI) => {
    try {
      const res = await todoApi.post('/tasks', newTask)
      return res.data as Task
    } catch (error) {
      console.log(error)
      return thunkAPI.rejectWithValue(error)
...
export const onUpdateTask = createAsyncThunk<Task, Partial<Task>>(
  ...
  async (updatedTask, thunkAPI) => {
    try {
      const res = await todoApi.put(`/tasks/${updatedTask.id}`, updatedTask)
      return res.data as Task
    } catch (error) {
      return thunkAPI.rejectWithValue(error)
    }
  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(onFetchTasks.rejected, (state, { payload }) => {
        state.loading = false
        state.backendErrorMessage = extractBackendErrorMessage(payload) || 'Error fetching tasks.'
      })
      // FETCH task by id
      ...
      .addCase(onFetchTasksByUserId.rejected, (state, { payload }) => {
        state.loading = false
        state.backendErrorMessage = extractBackendErrorMessage(payload) || 'Error fetching user tasks.'
      })
      // CREATE
      ...
      .addCase(onCreateTask.rejected, (state, { payload }) => {
        state.loading = false
        state.backendErrorMessage = extractBackendErrorMessage(payload) || 'Error creating task.'
      })
      // UPDATE
      ...
      .addCase(onUpdateTask.rejected, (state, { payload }) => {
        state.loading = false
        state.backendErrorMessage = extractBackendErrorMessage(payload) || 'Error updating task.'
      })
      // DELETE
      ...
      .addCase(onDeleteTask.rejected, (state, { payload }) => {
        state.loading = false
        state.backendErrorMessage = extractBackendErrorMessage(payload) || 'Error deleting task.'
      })
  }
})
